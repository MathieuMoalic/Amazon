...
class Game:
	...
	def play(self,WhiteAI,BlackAI):
		...
		self.winner = self.pieces[-self.playerTurn]
		print(self.winner,"won")
		self.reset()
		pygame.quit()

	#added for online
	def checkClick(self,mpos,board):
		#checks if the click from the player was on a valid tile
		#returns if it's valid, and the updated board
		gui = GUI(board)
		gui.draw(display=False)

		if self.stage == 1:
			print("stage 1")
			print("player turn :",self.playerTurn)
			queens = gui.listval(self.playerTurn)
			for queen in queens:
				if queen.rect.collidepoint(mpos) and self.stage==1:
					self.showPossibility([queen.pos[1],queen.pos[0]])
					self.queenPos = queen.pos
					self.stage = 2
					break
		...
		if self.stage == 2:
			print("stage 2")
			print("player turn :",self.playerTurn)
			redTiles = gui.listval(3)
			for redTile in redTiles:
				print("collides ?")
				if redTile.rect.collidepoint(mpos) and self.stage==2:
					self.board[self.queenPos[1],self.queenPos[0]]=0
					self.board[redTile.pos[1],redTile.pos[0]]=self.playerTurn
					self.queenPos = [redTile.pos[1],redTile.pos[0]]
					self.clearRedTiles()
		...
		if self.stage == 3:
			...
			redTiles = self.gui.listval(3)
			for redTile in redTiles:
				if redTile.rect.collidepoint(mpos) and self.stage==3:
					self.board[redTile.pos[1],redTile.pos[0]]=2
					self.clearRedTiles()
					self.stage = 1
					self.playerTurn *= -1
...
class GUI:
	def __init__(self,board):
		...
		self.board = board
		#pygame.init()
		path = "C:/rep/Games/Amazon/sprites/"
		# #dark tiles
		# self.dark_arrow = pygame.image.load(path + 'dark/arrow.png')
		# self.dark_bqueen = pygame.image.load(path + 'dark/bqueen.png')
		# self.dark_square = pygame.image.load(path + 'dark/square.png')
		# self.dark_wqueen = pygame.image.load(path + 'dark/wqueen.png')
		# self.dark_red = pygame.image.load(path + 'dark/red.png')
		# #clear tiles
		# self.clear_arrow = pygame.image.load(path + 'clear/arrow.png')
		# self.clear_bqueen = pygame.image.load(path + 'clear/bqueen.png')
		# self.clear_square = pygame.image.load(path + 'clear/square.png')
		# self.clear_wqueen = pygame.image.load(path + 'clear/wqueen.png')
		# self.clear_red = pygame.image.load(path + 'clear/red.png')		
...
if __name__ == "__main__":
    Game(size="tiny").play(False,True)
